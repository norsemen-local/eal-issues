...
| 4b | Rare ad domains | DNS + `GET px?cid=…` to 4 **invented** `.net` domains → NXDOMAIN **+** `test-adtracking`.testpanw.com | Rare access to advertising domains | 🟠 **SIMULATED** (fake domains aren't in PANW's ad category) · only the ✅ anchor fires | FW (EAL / DNS-Sec) |
...
to fire, and only if the attacker's port is actually listening. **Stage 4b's ad
beaconing is simulated** — the four `.net` names are invented and not in PANW's
advertising category, so its named detector won't fire; only the co-located
`test-adtracking` anchor produces a real detection. Stage 5 **does really create**
