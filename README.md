...
| 🟠 **SIMULATED (traffic-only)** | Only **approximates** the footprint — opens the surface or sends look-alike traffic but never performs the defining protocol call/exploit. | **Not by default** — the exact named detector won't fire on look-alike traffic alone. The AD/identity exploit stages (cases 4 & 9) **become REAL** when you run with **`-EnableRealExploits`** *and* drop the matching operator-supplied tool (mimikatz / Rubeus / Impacket / PetitPotam / AADInternals) in `scripts\tools\`. The co-located test-domain anchor always fires ✅. |
...
🟡 is real traffic that needs its baseline to mature or be seeded. The remaining 🟠
stages are simulated *by default* but can be made genuinely real: the case 4 & 9
credential-access exploits via the **`-EnableRealExploits`** opt-in + an
operator-supplied tool (this repo never bundles or downloads one), and the
target-dependent ones (real ad domains, real SSH handshake, categorised upload
destination) are now wired to real targets. **The scripts themselves still run no
real malware or memory-corruption exploit** — the web-attack *strings* in case 2 are
genuine, and the real AD/identity exploits execute only the audited tool you
explicitly supply and enable.

> **Hard limit — baseline analytics can't be forced instant.** The 🟡 behavioural/
> identity rules send *real* traffic, but no script change makes them fire on the
> first run: Cortex trains them over a **~30-day + 14-day** window. For a fresh-tenant
> demo, lean on the ✅ instant anchors (URL-Filtering + DNS-Security + endpoint
> BIOCs); to see the named behavioural rules, let the baseline mature or pre-seed it
> in the tenant. "Real traffic" ✅ and "fires instantly" are two different things.

### Per-case summary

| Case | What it does (lifecycle) | What you should see in XSIAM | ✅ Instant hits (first run) | 🟠 Simulated by default (→ how it becomes real) |
|------|--------------------------|------------------------------|----------------------------|-------------------------------------------------|
...
| **4 — AD Domination** | Phish → real LDAP enum + WPAD → then **EFSRPC, DCSync, Bronze Bit** (SMB/RPC/Kerberos surface by default; real exploit on opt-in). | Instant phishing hit; LDAP/WPAD analytics once baselined + ITDR; the three exploit detectors fire once you enable the real tool. | Phishing/malware URL-Filtering (stage 1). | **EFSRPC/PetitPotam, DCSync, Bronze Bit** → **REAL** with `-EnableRealExploits` + PetitPotam / mimikatz / Rubeus / Impacket in `scripts\tools\`. |
| **5 — Covert Exfil** | Phish → FTP anon+brute → uncommon SSH (real handshake) → SSH downgrade banner → oversized ICMP → 20 MB SMB copy. | Instant phishing + FTP anon/brute analytics; SSH host-key/downgrade & rare-SMB once baselined. | Phishing/malware URL-Filtering (stage 1). | **ICMP router-advert / multi-host / smurf** (needs a raw-socket tool + distinct IPs). *(SSH same-host-key is now a real `ssh.exe` handshake — fires if the lab servers share one host key.)* |
...
| **8 — Departing Employee** | *No phishing.* Insider browses job-boards/streaming → stages a real zip → new/rare **FTP** exfil → real **45 MB** upload (destination categorises as online-storage). | FTP New-Server/rare-user analytics (enabled) + DNS-Security instant; job-hunting & massive-upload analytics may be **off/baselined**. | `test-dns-infiltration`, `test-proxy`.testpanw.com. FTP rules fire near-real-time (enabled). | None — all traffic real; destination now categorises as online-storage (`mega.nz`). For the exact **volume** detector, point `StorageDomain` at a real storage host you control + set `UploadVolumeToStorage`. |
| **9 — Pass-the-Hash Playbook** | Phish → long-user/rare NTLM → machine-account NTLM → RC4 Kerberos TGT → ADFS sync / Golden SAML (real exploit on opt-in). **Needs a real AD/ADFS lab + ITDR.** | Instant phishing + DNS-Security anchors; identity analytics only with **ITDR + real AD/ADFS + baseline**; one endpoint BIOC (cert-store access). | Phishing + `test-c2`, `test-malware`, `test-dns-infiltration`.testpanw.com. | **RC4 downgrade** and **Golden SAML** → **REAL** with `-EnableRealExploits` + Rubeus / AADInternals or ADFSDump (operator creds + local ADFS access). |
| **10 — Tunnels & Shadows** | Phish → uncommon SSH (raw-socket banner) → ~640 KB "tunnel" volume → real known ad-domains → remote scheduled-task persistence, plus a file-assoc hijack. | Instant URL-Filtering + `dnstun`/`adtracking` DNS-Security; SSH & remote-task EAL analytics need baseline; endpoint BIOC (file-assoc), agent shadows the remote-task on the target. | Phishing + `test-dnstun`, `test-adtracking`.testpanw.com. | None — stage 4 now uses **real known advertising domains** (doubleclick, googlesyndication, …) so the ad category matches for real; only rarity is baseline. |
